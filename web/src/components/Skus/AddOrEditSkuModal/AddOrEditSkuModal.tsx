...
const _AddOrEditSkuModal: React.FC<Props> = ({ isOpen, onClose, onSubmit, initialValue }) => {
    ...
    <>
      {isOpen && (
        <div className="fixed top-0 left-0 w-full h-full bg-opacity-50 flex justify-center items-center bg-dark-mixed-300">
          <div className="bg-gray-50 text-black p-5 border rounded-xl shadow-md" ref={ref}>
            <h1 className="font-semibold text-black">{initialValue ? "Edit Sku" : "Add Sku"}</h1>
            <form onSubmit={handleSubmit(submitHandler)}>
              ...
              <div className="grid gap-2 mt-2 grid-cols-3">
                <button
                  className="p-2 rounded-md hover:bg-gray-200 transition-colors duration-300 border"
                  type="submit"
                >
                  Submit
                </button>
                <button
                  className="col-start-3 p-2 rounded-md hover:bg-gray-200 transition-colors duration-300 border"
                  type="button"
                  onClick={handleClose}
                >
                  Close
...
export const AddOrEditSkuModal = _AddOrEditSkuModal;
